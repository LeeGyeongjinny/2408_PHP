...
-- 이게 되긴 하는데 IFNULL로 체크가능해서 굳이 이렇게 할 필요는 없다
FROM employees
;
...
-- COUNT(컬럼) : 검색 결과 중 해당 컬럼의 값이 NULL이 아닌 레코드의 수를 출력
SELECT
	COUNT(fire_at)
	,COUNT(*)
FROM employees;
...
-- 레코드에 순위를 매겨 반환
-- 동일한 값이 있는 경우에도 각 행에 고유한 번호룰 부여
SELECT
	emp_id
	,salary
